...
export class CommonService {
  ...
  getOptions(): Observable<Options> {
    return this.http.get<Options>(
      `${environment.serverUrl}/api/options/server`
    );
  }
  ...
  report(
    ...
    if (id > 0) {
      options['params'] =
        type === reportType.CAMPAIGNS ? { campaign_id: id } : { site_id: id };
    }

    return this.http.get<any>(
      `${environment.apiUrl}/stats/report/${type}/${dateStart}/${dateEnd}`,
      options
    );
  }
  ...
  getReportsList(): Observable<ReportsList> {
    return this.http.get<ReportsList>(
      `${environment.apiUrl}/stats/report/list`
    );
  }
  ...
  getReport(id: string): Observable<any> {
    ...
    return this.http.get<any>(
      `${environment.apiUrl}/stats/report/${id}`,
      options
    );
  }
  ...
  ): Observable<PaginatorResponse<RefLink>> {
    const url =
      undefined === pageUrl
        ? `${environment.apiUrl}/ref-links`
        : (pageUrl &&
            environment.serverUrl.startsWith('https:') &&
  ...
  getRefLinkInfo(token: string): Observable<RefLinkInfo> {
    return this.http.get<RefLinkInfo>(
      `${environment.apiUrl}/ref-links/info/${token}`
    );
  }

  deleteRefLink(refLinkId: number): Observable<any> {
    return this.http.delete<any>(
      `${environment.apiUrl}/ref-links/${refLinkId}`
    );
  }

  getMedia(): Observable<Media> {
    return this.http.get<Media>(
      `${environment.apiUrl}/options/campaigns/media`
    );
  }

  getLoginPlaceholders(): Observable<Placeholders> {
    return this.http.get<Placeholders>(
      `${environment.serverUrl}/panel/placeholders/login`
    );
  }
  ...
  getAccessTokens(): Observable<AccessTokenResponse[]> {
    return this.http.get<AccessTokenResponse[]>(
      `${environment.authUrl}/personal-access-tokens`
    );
  }

  addAccessToken(
    token: AccessToken
  ): Observable<AccessTokenResponseWithSecret> {
    return this.http.post<AccessTokenResponseWithSecret>(
      `${environment.authUrl}/personal-access-tokens`,
      token
    );
  }
  ...
  deleteAccessToken(id: string): Observable<null> {
    return this.http.delete<null>(
      `${environment.authUrl}/personal-access-tokens/${id}`
    );
  }
}
