...
export class AddFundsDialogComponent extends HandleSubscription implements OnInit {
  ...
  async onChainChange (chainId) {
    ...
    if (this.unwrappers !== null) {
      ...
      if (unwrapper != null) {
        ...
        this.tokenContract = new this.web3.eth.Contract(this.abi,
  ...
  async onAccountChanges (accounts) {
    ...
    const balance = await this.tokenContract.methods.balanceOf(
      this.metamaskAccount).call()
  ...
  initializeFiats (): void {
    ...
    this.subscriptions.push(this.fiatForm.valueChanges.subscribe(
      () => {
  ...
  onFiatSubmit (): void {
    ...
    this.subscriptions.push(this.settings.saveInvoice(data).subscribe(
      (invoice: Invoice) => {
      ...
      (err) => {
        if (err.error.errors) {
          Object.keys(err.error.errors).
            forEach(key => this.fiatForm.get(key).setErrors({
  ...
  validNowPaymentsAmount (amount) {
    return isNumeric(amount) && amount >= this.nowPayments.minAmount &&
  ...
  async depositWrapped () {
    ...
    if (!amount || amount > parseFloat(this.metamaskAccountAds)) {
      ...
      this.isFormBeingSubmitted = true
      try {
        const result = await this.tokenContract.methods.unwrapMessage(
          (amount * 1e11).toFixed(0),
          '0x' + this.adsharesAddress.replace(new RegExp('-', 'g'), ''),
          '0x' + this.paymentMemo).send({ from: this.metamaskAccount })
