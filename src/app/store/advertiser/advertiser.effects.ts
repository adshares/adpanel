...
export class AdvertiserEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap((payload) => {
        ...
        return this.service.getCampaignsTotals(from, to).pipe(
          map(
            (campaignsTotals) => new LoadCampaignsTotalsSuccess(campaignsTotals)
          ),
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.service.getCampaignsConfig().pipe(
          map(
            (payload) =>
              new LoadCampaignsConfigSuccess(<CampaignsConfig>payload)
          ),
    ...
    this.actions$.pipe(
      ...
      switchMap((payload) =>
        this.service.saveCampaign(payload).pipe(
          switchMap((campaign) => {
            if (
              payload.basicInformation.status !==
              campaign.basicInformation.status
            ) {
  ...
  updateCampaignStatus = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(
        this.store$.select('state', 'advertiser'),
        ([action, user], state) => {
        ...
        return this.service.updateStatus(payload.id, payload.status).pipe(
          ...
          catchError((error) => {
            ...
            if (error.status === HTTP_UNPROCESSABLE_ENTITY) {
              ...
              errorMsg =
                "We weren't able to activate given campaign.\n" +
                errors.join('\n');
            } else {
