  ...
  implements OnInit, OnDestroy
{
  ...
  get canActivateCampaign(): boolean {
    return (
      this.currentCampaignStatus ===
        this.campaignStatusesEnum[
          this.campaignStatusesEnum.DRAFT
        ].toLowerCase() ||
      this.currentCampaignStatus ===
        this.campaignStatusesEnum[
          this.campaignStatusesEnum.SUSPENDED
        ].toLowerCase() ||
      this.currentCampaignStatus ===
        this.campaignStatusesEnum[
          this.campaignStatusesEnum.INACTIVE
        ].toLowerCase()
    );
  }
      ...
      .subscribe((campaigns: Campaign[]) => {
        ...
        if (undefined !== selectedCampaign) {
          ...
          this.bidStrategyService
            ...
            .subscribe((uuid) => {
              this.isDefaultBidStrategy =
                this.campaign.bidStrategy.uuid === uuid;
            });
  ...
  private prepareMediumLabel(campaign: Campaign): void {
    const medium =
      this.route.snapshot.data.media[campaign.basicInformation.medium];
    if (medium) {
  ...
  updateBudgetInfo(): void {
    ...
    const errors = validCampaignBudget(
      this.campaignsConfig,
      this.campaign,
      this.user
    );
    if (errors.length > 0) {
  ...
  updateTargeting(): void {
    ...
    if (
      Array.isArray(this.campaign.targeting.requires) &&
      Array.isArray(this.campaign.targeting.excludes)
    ) {
      ...
      this.advertiserService
        ...
        .subscribe(
          (medium) => {
            ...
            this.targeting = parseTargetingOptionsToArray(
              this.campaign.targeting,
  ...
  onCampaignStatusChange() {
    ...
    this.store.dispatch(
      new UpdateCampaignStatus({ id: this.campaign.id, status })
    );
  }
  ...
  updateConversionTableItems(): void {
    ...
    this.conversionTableItems = this.campaign.conversions.map(function (
      element
    ) {
      const statistic = statistics.find(
        (item) => campaignId === item.campaignId && element.uuid === item.uuid
      );
