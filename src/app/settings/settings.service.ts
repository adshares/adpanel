...
export class SettingsService {
  ...
  getBillingHistory (
    dateFrom: string, dateTo: string, types: number[], limit?: number,
    offset?: number): Observable<BillingHistory> {
  ...
  calculateWithdrawal (
    to: string, amount?: number): Observable<CalculateWithdrawalItem> {
    return this.http.post<CalculateWithdrawalItem>(
      `${environment.apiUrl}/calculate-withdrawal`, { to, amount })
  }

  withdrawFunds (
    to: string, amount: number, memo: string, currency: string = 'ADS') {
